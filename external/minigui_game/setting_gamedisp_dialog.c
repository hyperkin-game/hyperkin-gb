...
static LRESULT setting_gamedisp_dialog_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    HDC hdc;
    ...
    case MSG_INITDIALOG: {
        ...
        batt = battery;
        list_sel = 0;
        SetTimer(hWnd, _ID_TIMER_SETTING_GAMEDISP, TIMER_SETTING_GAMEDISP);
        return 0;
    }
    case MSG_TIMER: {
        if (wParam == _ID_TIMER_SETTING_GAMEDISP) {
            if (batt != battery) {
                batt = battery;
    ...
    case MSG_PAINT:
    {
        ...
        DrawText(hdc, res_str[RES_STR_TITLE_GAMEDISP], -1, &msg_rcTitle, DT_TOP);
        FillBox(hdc, TITLE_LINE_PINT_X, TITLE_LINE_PINT_Y, TITLE_LINE_PINT_W, TITLE_LINE_PINT_H);

        page = (GAMEDISP_NUM + SETTING_NUM_PERPAGE - 1) / SETTING_NUM_PERPAGE;
        cur_page = list_sel / SETTING_NUM_PERPAGE;

        for (i = 0; i < SETTING_NUM_PERPAGE; i++) {
            RECT msg_rcFilename;

            if ((cur_page * SETTING_NUM_PERPAGE + i) >= GAMEDISP_NUM)
                break;
            ...
            if ((cur_page * SETTING_NUM_PERPAGE + i) == get_gamedisp())
            ...
            DrawText(hdc, res_str[RES_STR_TITLE_GAMEDISP_FULL + cur_page * SETTING_NUM_PERPAGE + i], -1, &msg_rcFilename, DT_TOP);
        }
    ...
    case MSG_DESTROY:
        KillTimer(hWnd, _ID_TIMER_SETTING_GAMEDISP);
        unloadres();
        break;
    }
...
void creat_setting_gamedisp_dialog(HWND hWnd)
{
    ...
    DialogBoxIndirectParam(&DesktopDlg, hWnd, setting_gamedisp_dialog_proc, 0L);
}