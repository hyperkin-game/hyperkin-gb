...
V4l2Device::request_buffer ()
{
    ...
    XCAM_LOG_INFO ("request buffers in device(%s)", XCAM_STR (_name));
    ...
    const uint32_t index)
{
    ...
    XCAM_LOG_INFO ("allocate buffers in device(%s), mem type: %d", XCAM_STR (_name), _memory_type);
    switch (_memory_type) {
...
V4l2Device::init_buffer_pool ()
{
    ...
    XCAM_LOG_INFO ("init buffer poll in device(%s), cnt: %d", XCAM_STR (_name), _buf_count);
    for (; i < _buf_count; i++) {
