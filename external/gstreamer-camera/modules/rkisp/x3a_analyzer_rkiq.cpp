...
X3aAnalyzerRKiq::X3aAnalyzerRKiq (struct atomisp_sensor_mode_data &sensor_data, const char *cpf_path)
    : X3aAnalyzer ("X3aAnalyzerRKiq")
...
{
    SmartPtr<RKiqAeHandler> ae_handler = new RKiqAeHandler (_rkiq_compositor);
...
{
    SmartPtr<AiqAwbHandler> awb_handler = new AiqAwbHandler (_rkiq_compositor);
...
X3aAnalyzerRKiq::configure_3a ()
{
    ...
    XCAM_LOG_DEBUG ("X3aAnalyzerRKiq got sensor mode data, coarse_time_min:%u, "
...
X3aAnalyzerRKiq::pre_3a_analyze (SmartPtr<X3aStats> &stats)
{
    ...
    if (!_rkiq_compositor->set_3a_stats (isp_stats)) {
