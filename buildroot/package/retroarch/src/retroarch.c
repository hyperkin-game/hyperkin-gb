      ...
      unsigned *sleep_ms)
{
   ...
   static unsigned old_ratio_status = 0xff;
   ...
   /* Check ratio button */
   {
	static unsigned ratio_button = 0;
	static int ratio_count = 0;
	char ratio_buf[2];
	if(old_ratio_status == 0xff){
		ratio_fd = open("/sys/devices/platform/adc-vol/ratio", O_NONBLOCK, O_RDONLY);
	}
	if(ratio_fd >= 0 && ratio_count > 10){
		memset(ratio_buf, '\0', sizeof(ratio_buf));
		lseek(ratio_fd, 0, SEEK_SET);
		read(ratio_fd, ratio_buf, 1);
		ratio_button = atoi(ratio_buf);
		ratio_count = 0;
	}
	if( (old_ratio_status != ratio_button) ){
		memset(ratio_buf, '\0', sizeof(ratio_buf));
...
volatile int g_reinit_audio = 0;
volatile int g_reinit_video = 0;
...
int runloop_iterate(unsigned *sleep_ms)
{
   ...
   if (g_reinit_video) {
      g_reinit_audio = g_reinit_video = 0;
      command_event(CMD_EVENT_REINIT, NULL);
   } else if (g_reinit_audio) {
      g_reinit_audio = 0;
