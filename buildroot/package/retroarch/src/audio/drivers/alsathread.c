...
static void alsa_worker_thread(void *data)
{
   ...
   extern void audio_thread_bind_cpu();
   audio_thread_bind_cpu();
      ...
      unsigned *new_rate)
{
   ...
   strncpy(alsa_dev, device ? device : "default", sizeof(alsa_dev));
   alsa_thread_override_device(alsa_dev, sizeof(alsa_dev));
