...
static int drm_override_mode(unsigned int *width, unsigned int *height)
{
   const char *path;
   char buf[256] = "\0";
   int fd, w, h;
...
nextgpu:
   ...
   drm_override_mode(&drm->fb_width, &drm->fb_height);
   for (i = 0; i < g_drm_connector->count_modes; i++)
   {
      if (drm->fb_width == g_drm_connector->modes[i].hdisplay &&
            drm->fb_height == g_drm_connector->modes[i].vdisplay)
         break;
   }
   if (i == g_drm_connector->count_modes) {
