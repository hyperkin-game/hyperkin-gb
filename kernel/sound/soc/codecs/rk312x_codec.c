...
static int rk312x_probe(struct snd_soc_codec *codec)
{
	...
	schedule_delayed_work(&rk312x_priv->init_delayed_work, msecs_to_jiffies(100));
...
static int rk312x_platform_probe(struct platform_device *pdev)
{
	...
	rk312x->av_ctl_gpio = devm_gpiod_get_optional(&pdev->dev, "av-ctl", GPIOD_OUT_LOW);
	if (!IS_ERR_OR_NULL(rk312x->spk_ctl_gpio)) {
		DBG(KERN_INFO "%s : av-ctl-gpio %d\n", __func__,
		    desc_to_gpio(rk312x->av_ctl_gpio));
	}else{
		gpiod_set_value(rk312x->av_ctl_gpio, 1);
	}
