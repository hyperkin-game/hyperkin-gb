...
#if 1
#define BATT_MAX_VOL_VALUE	16418/*Full  charge volatge*/
#define BATT_ZERO_VOL_VALUE	11348/*power down voltage*/
#define BATT_NOMAL_VOL_VALUE                         15000
...
{
	struct rk30_adc_battery_data *bat=  power_supply_get_drvdata(psy);
...
static struct power_supply_desc rk30_ac_supply = 
{
	.name = "ac",
...
static struct power_supply_desc rk30_battery_supply = 
{
	.name = "battery",
...
rk30_adc_battery_platform_data *data)
{
	...
	if (IS_ERR(channels)) {
		pr_err("get adc channels fails %ld\n", PTR_ERR(channels));
...
static int rk30_adc_battery_probe(struct platform_device *pdev)
{
	...
	data ->bat = devm_power_supply_register(&pdev->dev, &rk30_battery_supply,
						&psy_cfg);

#if  defined (CONFIG_BATTERY_RK30_USB_CHARGE)	
 	data ->usb = devm_power_supply_register(&pdev->dev, &rk30_battery_supply,
						&psy_cfg);
#endif
	data ->ac = devm_power_supply_register(&pdev->dev, &rk30_ac_supply,
						&psy_cfg);
