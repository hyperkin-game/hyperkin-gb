...
select_chip_info()
{
	...
	# Very hacky for 3128 games
	if grep  -q "CONFIG_DEBUG_UART_BASE=0x20064000" ${OUTDIR}/.config; then
		UART='_UART1'
	fi
}
...
# <*> Fixup rsa/sha pack mode for platforms
	...
	if [ $RKCHIP = "RK3128" -o $RKCHIP = "PX3SE" ]; then
...
pack_loader_image()
{
	...
	if [ ! -f ${RKBIN}/RKBOOT/${RKCHIP_LOADER}MINIALL${UART}.ini ]; then
		echo "pack loader failed! Can't find: ${RKBIN}/RKBOOT/${RKCHIP_LOADER}MINIALL${UART}.ini"
		return
	fi
	...
	if [ "${mode}" = 'all' ]; then
		files=`ls ${RKBIN}/RKBOOT/${RKCHIP_LOADER}MINIALL_UART1*.ini`
		for ini in $files
		do
			if [ -f "$ini" ]; then
		...
		${RKTOOLS}/boot_merger ${BIN_PATH_FIXUP} ${RKBIN}/RKBOOT/${RKCHIP_LOADER}MINIALL${UART}.ini
		echo "pack loader okay! Input: ${RKBIN}/RKBOOT/${RKCHIP_LOADER}MINIALL${UART}.ini"
	fi
...
pack_trust_image()
{
	local TOS TOS_TA DARM_BASE TEE_LOAD_ADDR TEE_OFFSET=0x1000000
	...
	# ARM uses loaderimage
		...
		if [ ! -f ${RKBIN}/RKTRUST/${RKCHIP_TRUST}TOS${UART}.ini ]; then
			echo "pack trust failed! Can't find: ${RKBIN}/RKTRUST/${RKCHIP_TRUST}TOS${UART}.ini"
			return
		fi

		# OP-TEE is 16M(0x1000000) offset from DRAM base.
		...
		# Parse orignal path
		TOS=`sed -n "/TOS=/s/TOS=//p" ${RKBIN}/RKTRUST/${RKCHIP_TRUST}TOS${UART}.ini|tr -d '\r'`
		TOS_TA=`sed -n "/TOSTA=/s/TOSTA=//p" ${RKBIN}/RKTRUST/${RKCHIP_TRUST}TOS${UART}.ini|tr -d '\r'`
		...
		echo "pack trust okay! Input: ${RKBIN}/RKTRUST/${RKCHIP_TRUST}TOS${UART}.ini"
	fi
}

pack_rk3128_idbloader()
{
	echo -e "\nPacking idbloader.img for sdboot"
	set -e
	./tools/mkimage -n rk3128 -T rksd -d ../rkbin/bin/rk31/rk3128_ddr_300MHz_v2.08_uart1_20180921.bin idbloader.img
	cat ../rkbin/bin/rk31/rk312x_miniloader_v2.52.bin >> idbloader.img
	set +e
	echo "Packing idbloader.img done"
}
